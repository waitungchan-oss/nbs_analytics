...
def _catalog_fingerprint(payload: Mapping[str, Any], fingerprint_key: str) -> str:
    return _sha(payload[fingerprint_key], fingerprint_key)
...
class GovernanceGraphOwnerCatalog:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "GovernanceGraphOwnerCatalog":
        ...
        for index, raw in enumerate(entries_raw):
            ...
            entries.append(MappingProxyType({"subject": subject, "owner": MappingProxyType({"kind": owner["kind"], "id": owner["id"]}), "source": entry_source, "snapshotFingerprint": entry_snapshot, "status": entry_status}))
...
def _diagnostics(value: Any, key: str) -> tuple[MappingProxyType, ...]:
    ...
    for index, item in enumerate(value):
        ...
        result.append(MappingProxyType({"code": _identifier(data["code"], f"{key}[{index}].code"), "summary": _identifier(data["summary"], f"{key}[{index}].summary")}))
    return tuple(result)
