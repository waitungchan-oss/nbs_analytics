...
def test_registry_preserves_exact_monthly_values_and_six_month_total():
    ...
    assert [row["mode"] for row in registry["baselines"]] == [
        "monitoring",
        "monitoring",
        "monitoring",
        "monitoring",
...
def test_evaluate_monthly_baselines_matches_all_six_months():
    ...
    result = evaluate_monthly_baselines(analytics_builder=_analytics_builder())
...
def test_legacy_may_blocking_check_is_not_duplicated_in_core_gate():
    ...
    evaluation = evaluate_monthly_baselines(analytics_builder=_analytics_builder())
    result = apply_monthly_blocking_checks(_base_gate(), evaluation)
...
def test_promotion_rejects_stale_upload_record(tmp_path, monkeypatch):
    ...
    registry_path.write_text(json.dumps(service.load_monthly_baseline_registry(), ensure_ascii=False), encoding="utf-8")
    evaluation = service.evaluate_monthly_baselines(analytics_builder=_analytics_builder())
    monkeypatch.setattr(
        service,
        "build_monthly_baseline_governance",
...
def test_promotion_updates_only_monitoring_months_and_records_audit(tmp_path, monkeypatch):
    ...
    registry_path = tmp_path / "baselines.json"
    original = service.load_monthly_baseline_registry()
